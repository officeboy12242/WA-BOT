...
const INDEX_SYMBOL = '^NSEI';
const VOLUME_PROXY = 'NIFTYBEES.NS';
 ...
 */
export async function fetchNiftySessionBars({ force = false } = {}) {
    if (!force && _cache && Date.now() - _cache.at < CACHE_TTL_MS) return _cache.value;

    let indexBars;
    try {
        indexBars = await fetchYahooIntradayCandles(INDEX_SYMBOL, { interval: '5m', range: '5d' });
    } catch (err) {
        logger.warn(`NIFTY session bars: index fetch failed — ${err.message}`);
        return null;
    }
    ...
    let volByTs = null;
    try {
        const etf = await fetchYahooIntradayCandles(VOLUME_PROXY, { interval: '5m', range: '5d' });
        if (etf?.length) {
            volByTs = new Map();
            for (const b of etf) if (b.volume > 0) volByTs.set(b.ts, b.volume);
        }
    } catch (err) {
        logger.debug(`NIFTY session bars: volume proxy unavailable — ${err.message}`);
    }
    ...
    const value = { bars, sessionDate, volumeSource, barCount: bars.length, volumeBars: matched };
